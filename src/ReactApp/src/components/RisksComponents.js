  ...
  render() {
    ...
    return (
      <div>
        ...
        <Modal
          centered
          title={this.props.t("Assets.analysis.risks.add")}
            ...
            {this.state.step === 0 && (
              <div>
                ...
                <Form.Item label={this.props.t("Assets.analysis.risks.addLinks")}>
                  <Select mode="tags" onChange={this.connectObjects}>
                    {this.props.asset.vulnerabilities.map((item, index) => {
                      return (
                        <Select.Option key={"vulnerabilities." + item.name} value={"v:" + item.id}>
                    ...
                    {this.props.asset.risks.map((item, index) => {
                      return (
                        <Select.Option key={"risks." + item.name} value={"r:" + item.id}>
                          Risks:{item.name}
                        </Select.Option>
                      );
                    })}
                ...
                <Form.Item label={this.props.t("Assets.analysis.risks.lindun")}>
                  <Select onChange={index => this.setLindun(index)}>
                    {this.lindun.map(s => (
                      <Select.Option key={s}>{s}</Select.Option>
                    ))}
                  </Select>
                          ...
                          <Col span={12} key={subgroup.name}>
                            ...
                            {subgroup.children.map(subsubgroup => (
                              <Row key={subsubgroup.name}>
                                ...
                                <Col span={23}>
                                  <span>
                                    <Slider min={1} max={9} onChange={v => this.pushOwaspModel(group.name, subgroup.name, subsubgroup.name, v)} />
                                  </span>
                                </Col>
                              </Row>
                            ))}
                  ...
                  <div>
                    <Form.Item
                      label={
                        <span>
                          Likelihood{" "}
                          <Select onChange={v => this.setNewValue("likelihood", v)} defaultValue="low">
                      ...
                      <Input.TextArea rows={2} id="likelihoood" onChange={e => this.setNewValue("likelihoodText", e.target.value)} placeholder="Describe the likelihood here..." />
                    </Form.Item>
                    <Form.Item
                      label={
                        <span>
                          Impact:{" "}
                          <Select onChange={v => this.setNewValue("impact", v)} defaultValue="low">
                            <Select.Option key="low">LOW</Select.Option>
                            <Select.Option key="medium">MEDIUM</Select.Option>
                            <Select.Option key="high">HIGH</Select.Option>
                          </Select>
                      ...
                      <Input.TextArea rows={2} id="impact" onChange={e => this.setNewValue("impactText", e.target.value)} placeholder="Describe the impact here..." />
                    </Form.Item>
                  </div>
                )}
              </div>
            ...
            {this.state.step === 2 && (
              <div>
                ...
                <Row gutter={16} style={{ marginTop: "20px" }}>
                  {this.state.newEntry.treatments.map((treatment, index) => (
                    <Col span={12} key={`treatment.${index}`}>
                      <Card
                        style={{ marginBottom: "15px" }}
                        title={`Type: ${this.props.t(`Assets.analysis.treatments.${treatment.type}`)}`}
